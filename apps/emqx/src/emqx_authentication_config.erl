...
do_pre_config_update(Paths, NewConfig, OldConfig) ->
    ...
    NewConfig1 =
        lists:map(
            fun(New) ->
                NewId = authenticator_id(New),
                case
                    lists:filter(
                        fun(OldConfig0) -> authenticator_id(OldConfig0) =:= NewId end, OldConfig
                    )
                of
                    [] ->
...
do_post_config_update(_, _UpdateReq, OldConfig, OldConfig, _AppEnvs) ->
    ok;
do_post_config_update(Paths, _UpdateReq, _NewConfig, _OldConfig, _AppEnvs) ->
    _ChainName = chain_name(Paths),
    ok.
